...
meas dc vout_dc_high max v(vout)
meas dc vout_dc_low  min v(vout)
meas dc vtrip_nom find v(vin) when v(vout)=1.65 cross=1

let vos_nom = vtrip_nom - 1.7
...
*--------------------------------------------------
* 2. Transient, delay, power and hysteresis
*--------------------------------------------------
tran 10n 420u

write comparator_all_tran.raw all

* Fast switching section: 0 to 120 us
...
* Inverting output:
meas tran tpd_rise trig v(vin) val=1.7 rise=1 td=5u targ v(vout) val=1.65 rise=1 td=5u
meas tran tpd_fall trig v(vin) val=1.7 fall=1 td=20u targ v(vout) val=1.65 fall=1 td=20u
...
*--------------------------------------------------
* 3. Hysteresis section: 200 to 400 us
* Slow Vin rises from 200 to 300 us,
* then falls from 300 to 400 us.
* For an inverting output:
* rising Vin gives falling Vout,
* falling Vin gives rising Vout.
*--------------------------------------------------
meas tran vtrip_up find v(vin) when v(vout)=1.65 rise=1 td=200u
meas tran vtrip_down find v(vin) when v(vout)=1.65 fall=1 td=300u
