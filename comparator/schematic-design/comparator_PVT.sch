...
N -270 250 330 250 {lab=vin}
N -310 200 -310 220 {lab=vin}
N -310 200 -240 200 {lab=vin}
N -240 200 -240 250 {lab=vin}
N -180 170 -180 320 {lab=0}
N -150 140 700 140 {lab=#net1}
N -180 40 -180 110 {lab=#net1}
N -180 90 -100 90 {lab=#net1}
N -100 90 -100 130 {lab=#net1}
N -100 130 -100 140 {lab=#net1}
N -420 70 -40 70 {lab=vref}
N -420 -170 -120 -170 {lab=VDD}
N -180 -170 -180 -20 {lab=VDD}
N -310 150 -310 210 {lab=vin}
...
N 700 140 870 140 {lab=#net1}
N 870 140 880 140 {lab=#net1}
...
N -560 210 830 210 {lab= vin
}
N -110 90 -100 90 {lab=#net1}
N -250 200 -240 200 {lab=vin}
N 370 210 370 220 {lab=vin}
...
C {vsource.sym} -560 240 0 0 {name=Vin value=1.7 savecurrent=false}
...
.temp 25"}
C {code_shown.sym} -1440 -100 0 0 {name=NGSPICE only_toplevel=false value=".control
set noaskquit
save all

tran 10n 120u

write comparator_pvt_tt.raw all

meas tran vout_high_tt max v(vout) from=5u to=120u
meas tran vout_low_tt min v(vout) from=5u to=120u

meas tran tpd_hl_tt trig v(vin) val=1.7 rise=1 targ v(vout) val=1.65 fall=1
meas tran tpd_lh_tt trig v(vin) val=1.7 fall=1 targ v(vout) val=1.65 rise=1

meas tran idd_avg_tt avg i(VDD) from=10u to=120u
let pavg_tt = abs(3.3 * idd_avg_tt)
