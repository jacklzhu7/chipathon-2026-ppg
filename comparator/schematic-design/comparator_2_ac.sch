...
C {vsource.sym} -420 100 0 0 {name=Vref value="SIN(1.7 20m 1k 0 0 180)" savecurrent=false}
C {gnd.sym} -420 130 0 0 {name=l4 lab=0}
C {vsource.sym} -560 240 0 0 {name=Vin value="SIN(1.7 20m 1k 0 0 0)
" savecurrent=false}
...
let vdiff = v(vin)-v(vref)

write comparator_two_sine.raw all

* Plot 1: two opposite-phase comparator inputs
plot v(vin) v(vref) title 'Comparator Differential Inputs'

* Plot 2: differential input voltage
plot vdiff title 'Differential Input: Vin - Vref'

* Plot 3: comparator output
plot v(vout) title 'Comparator Output'

* Plot 4: inputs and output together
plot v(vin) v(vref) v(vout) title 'Comparator Transient Response'

* Plot 5: output versus differential input
plot v(vout) vs vdiff title 'Comparator Transfer Characteristic'
